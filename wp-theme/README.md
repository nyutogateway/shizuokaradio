...
**お知らせ一覧は2通りあります。どちらか一方だけにしてください。**

| 方法 | 手順 | 使われるファイル |
|---|---|---|
| A. WordPress標準（推奨） | 設定 > 表示設定 > 投稿ページ に空の固定ページを指定 | `home.php` |
| B. 固定ページで揃える | 固定ページを作り、テンプレートに「お知らせ一覧」を選ぶ（スラッグは `news`） | `template-news.php` |

Aが標準的ですが、他の一覧（パーソナリティー／番組）と管理方法を揃えたい場合はBでも構いません。
**両方やると2つのお知らせ一覧ができてしまう**ので、どちらか一方にしてください。
