...
帯に出る大きな英字（NEWS / PROGRAM など）は、**選んだテンプレートから自動で決まります**。

| テンプレート | 英字ラベル |
|---|---|
| お知らせ一覧 | NEWS |
| パーソナリティー一覧 | PERSONALITY |
| 番組一覧 | PROGRAM |
| リクエストフォーム | REQUEST |
| 会社概要 | COMPANY |

専用テンプレートが無いページ（プライバシーポリシー等）は既定で **PAGE** になります。
変えたい場合は、固定ページ編集画面の右側「**ページヘッダーの英字ラベル**」に入力してください
（例：`POLICY`）。
...
記事は**「投稿」で管理**し、一覧は**固定ページ＋「お知らせ一覧」テンプレート**で表示します。
スラッグは `news` にしてください。

> **設定 > 表示設定 > 投稿ページ は指定しないでください。**
> 指定するとそちらが優先され、お知らせ一覧が2つできてしまいます（`home.php` が使われる）。
> 空欄のままにしておけば、固定ページ側だけが使われます。

**3つの一覧ページまとめ**

| 固定ページ | スラッグ | 選ぶテンプレート | 記事の管理場所 |
|---|---|---|---|
| お知らせ | `news` | お知らせ一覧 | **投稿** |
| パーソナリティー | `personality` | パーソナリティー一覧 | パーソナリティー（CPT） |
| 番組一覧 | `program` | 番組一覧 | 番組（CPT） |

スラッグが違っても、テンプレートからページを探すのでリンクは繋がります（`sor_list_url()`）。
